      ...
      <section>
        ...
        <div className="rounded-2xl border border-rose-300 bg-rose-50 p-6 text-center">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            This may be a database configuration issue. Please contact the administrator.
          </p>
        </div>
      </section>
    );
    ...
    <section>
      <header className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Information shown to editors and used as default author details.
        </p>
        {roles.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {roles.map((r) => (
              <span
                key={r}
                className="rounded-full bg-primary/10 px-3 py-1 text-[11px] font-medium uppercase tracking-wide text-primary"
              >
                {r}
              </span>
            ))}
      ...
      <form onSubmit={save} className="space-y-4 rounded-2xl border border-border/60 bg-card p-6">
        ...
        <Field label="Affiliation" name="affiliation" defaultValue={profile.affiliation ?? ""} />
